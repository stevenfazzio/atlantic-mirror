"""Stage 07: caption each displayed (European city, US city) pair with a shared-character phrase.
...
caption is direction-agnostic, so each European<->US pair is captioned ONCE (normalized to
European-first) and attached to the match in both directions. Cached per pair (resumable).

Reads:  data/processed/matches_<model>[_profile_<key>].json, data/processed/cities.parquet
Writes: data/raw/captions/<eu_qid>__<us_qid>.txt                             cached per pair
...
SYSTEM = """You are labeling a pair of "sibling cities" -- a European city and an American city an \
algorithm identified as cross-country analogs -- for a general audience.

You will be given both cities' encyclopedia leads. Write a single concise descriptive PHRASE (not \
a full sentence; ~10-20 words) capturing the shared character the two cities have -- the kind of \
place they both are. It must read as one description that applies equally to each.
...
def make_caption(eu_lead: str, us_lead: str) -> str:
...
def caption_pair(eu_qid, us_qid, eu_lead, us_lead, *, force):
    path = RAW / "captions" / f"{eu_qid}__{us_qid}.txt"
    if path.exists() and not force:
        return (eu_qid, us_qid), path.read_text()
    text = make_caption(eu_lead, us_lead)
    path.parent.mkdir(parents=True, exist_ok=True)
    tmp = path.with_suffix(".txt.tmp")
    tmp.write_text(text)
    tmp.replace(path)
    return (eu_qid, us_qid), text


def norm_pair(rec_qid: str, group: str, m_qid: str) -> tuple[str, str]:
    """Normalize a (city, match) pair to (european_qid, us_qid) regardless of direction."""
...
def main() -> None:
    ...
    print(f"{len(pairs)} unique European<->US pairs to caption")

    results, done = {}, 0
    with ThreadPoolExecutor(max_workers=args.workers) as ex:
        futs = [
            ex.submit(caption_pair, euq, usq, lead[euq], lead[usq], force=args.force)
            for euq, usq in pairs
        ]
