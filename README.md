...
country signal. Matching then runs on the country-neutralized profile embeddings with CSLS
(which corrects for hub cities); each UK city gets its top-3 US analogs with cosine weights.
...
| 06 | `06_map.py` | stage 05 + cities | `output/uk_map_<model>[_profile_<key>].html` |

Stages 03–06 take `--source {lead,profile}` (and `--profile-key`, e.g. `haiku`/`opus`): the
**profile** track is the primary representation; the **lead** track is kept as a control for
comparison. Stage 02b takes `--model`/`--key` to distill with different LLMs. Stage 04 builds
three neutralized representations (`raw_pca`, `centroid`, `leace`) with diagnostics; stage 05
reads one (default `centroid`).

Raw API responses are cached verbatim under `data/raw/` (SPARQL, leads, one file per distilled
profile) so we never re-hit the network or re-pay an LLM call. Stage 02b needs
`ANTHROPIC_API_KEY`.
...
Drop `--source profile` from 03–06 for the lead-based control track. Add `--force` to ignore
a stage's cache.
...
name-fix experiments (output surgery, masking, name-vector / subspace projection) plus shared
helpers used by `eval_profiles.py`.
